...
export default function Anns() {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await axios.post("/api/announcement", {
  ...
  const handleDelete = async (e) => {
    e.preventDefault()
    const res = await axios.delete(`/api/announcement/${formData.annId}`)
    dispatch(deleteModalClose())
    console.log(res)
    setFormData({annId:"",annTitle:"",annText:""})
    setFile(undefined)
            ...
            <div className="createone-rel">
              ...
              <fieldset>
                <div className="file_ch">
                  <input type="file" className='file-field' placeholder='ann img' name='file' onChange={(e) => setFile(e.target.files[0])}   />
                  {file ? "File chosen" : "Choose a file"}
                </div>
                <input type="text" placeholder='ann title' name='annTitle' value={formData.annTitle} onChange={(e) => setFormData(prev => {return {...prev, [e.target.name] : e.target.value}})}  />
                <input type="text" placeholder='ann text' name='annText' value={formData.annText} onChange={(e) => setFormData(prev => {return {...prev, [e.target.name] : e.target.value}})} />
